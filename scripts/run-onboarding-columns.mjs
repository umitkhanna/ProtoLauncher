/**
 * Apply scripts/mysql-auth-onboarding-columns.sql (idempotent-ish: ignores duplicate column errors).
 */
import fs from "fs";
...
async function main() {
  ...
  const sqlPath = path.join(__dirname, "mysql-auth-onboarding-columns.sql");
  ...
  try {
    ...
    console.log("Onboarding columns applied on", database);
  } catch (e) {
    if (String(e?.message || e).includes("Duplicate column name")) {
      console.log("Onboarding columns already exist; skipping.");
      return;
    }
    throw e;
  } finally {
