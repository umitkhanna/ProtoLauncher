...
/**
 * Converts model output to HTML (handles fenced ```html blocks or raw Markdown).
 */
...
/**
 * @param {object} projectRow from getProjectByIdAndUser (name, description, startup_idea, …)
 * @returns {Promise<string>} raw model output (before HTML normalization)
 */
export async function generateRequirementsSpecRaw(projectRow) {
  ...
  if (!provider) {
    throw new Error(
      "No AI provider configured. Add ANTHROPIC_API_KEY and/or OPENAI_API_KEY to .env.local, or in queue-api/.env for the same keys the worker uses.",
    );
  }
